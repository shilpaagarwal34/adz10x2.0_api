...
AWS.config.update({
 ...
 exports.getSocietyRegistration = async (req, res) =>{
     try {
 ...
 exports.getSocietyRegistrationById = async (req, res) => {
     try {
          const SocietyRegistration = await Society_Registration.findByPk(req.params.id);
          res.json(SocietyRegistration)
     } catch (err) {
          res.status(500).json({ message: "Register not found" })
     }
 ...
 exports.societyRegistration = async (req, res) => {
    try {
        ...
        let { society_name, name, mobile_number, email, city_id, area_id, area_name, pincode, address, latitude, longitude, is_agree_terms_condition } = req.body;
        ...
        // const emailExists = await Society_Registration.findOne({ where: { email }, status: { [Op.ne]: 'delete' } });
        // const mobileExists = await Society_Registration.findOne({ where: { mobile_number } , status: { [Op.ne]: 'delete' } });

        const emailExists = await Society_Registration.findOne({
                    where: {
                        email,
        ...
        const mobileExists = await Society_Registration.findOne({
                where: {
                    mobile_number,
                    status: { [Op.ne]: 'delete' }   // not equal to 'deleted'
                }
        ...
        // Store society registration with updated area_id
        const newSociety = await Society_Registration.create({
            society_name,
        ...
        if (!newSociety || typeof newSociety.id === 'undefined') {
            return res.status(500).json({ status: 500, message: "Failed to create society (no ID returned)" });
        }

        const userType = 'Society_Admin';
        
        const token = jwt.sign(
            { email, id: newSociety.id, userType },
            process.env.JWT_SECRET,
            { expiresIn: '7d' }
        );

        // 4. Update the company record with the generated token
        await newSociety.update({ token });

        const formattedId = newSociety.id < 10 ? `0${newSociety.id}` : `${newSociety.id}`;
        const generatedPrefix = `ADZ10XSA${formattedId}`;

        await newSociety.update({ id_prifix_society: generatedPrefix });

                // Create Society_Profile using newSociety.id
        const newSocietyProfile = await Society_Profile.create({
            society_id: newSociety.id, 
            created_ip_address: req.ip,
            created_by: newSociety.id, 
            number_of_members: '',
            status: 'active'
        });
       ...
       let notification = await Notification.create({
            society_ids: [newSociety.id], // Ensure this is handled as JSON in DB
            message: `New society "${society_name}" has been registered.`,
            from: 'society',
            to: 'admin',
