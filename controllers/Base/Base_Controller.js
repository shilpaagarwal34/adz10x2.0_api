...
exports.commonDelete = async (req, res) => {
    ...
    // Validate input
    if (!table || !id) {
      return res.status(400).json({ status: false, message: "Table and ID are required." });
    }
...
exports.truncateAllTablesWithIds = async (req, res) => {
  try {
    ...
    await Society_Registration.truncate({ restartIdentity: true, cascade: true });
    await Company_Registration.truncate({ restartIdentity: true, cascade: true });
    ...
    await Campaign_Configuration.truncate({ restartIdentity: true, cascade: true });
    ...
    await Society_Wallet_Payment.truncate({ restartIdentity: true, cascade: true });
    ...
    await Society_Withdraw_Payments.truncate({ restartIdentity: true, cascade: true });
...
exports.getSecotrs  = async (req, res) => {
    try{
         ...
         return res.status(200).json({ status:200, message: 'Sectors fetched successfully', data: sectors });
    }catch(err){ 
         return res.status(500).json({ status:500, error:error.message })
    }
}
    ...
    try {
        ...
        if (!user) {
            try {
                ...
                if (result) ({ tempUser: user, typeName: userType, profileKey: profileImageKey, model: modelType } = result);
            } catch (err) {
                if (err.message === "INVALID_PASSWORD") return res.status(401).json({ status: 401, message: "Invalid password" });
                if (err.message === "USER_INACTIVE") return res.status(403).json({ status: 403, message: "User is inactive. Please contact support." });
            }
        }
        ...
        if (!user) {
            return res.status(400).json({ status: 400, message: "User not found with this email." });
        }
        ...
        if (modelType === Society_Registration || modelType === Company_Registration) {
            if (user.is_otp_verified === "0") {
                let otp, isOtpUnique = false;
        ...
        if (userType === 'Society_User' || userType === 'Company_User' || userType === 'Company_Admin' || userType === 'Society_Admin') {
            await user.update({
...
exports.loginUser = async (req, res) => {
    try {
        ...
        user = await Society_Registration.findOne({ where: { email, status: 'active' } });
        if (user) {
        ...
        if (!user) {
            user = await Company_Registration.findOne({ where: { email, status: 'active' } });
            if (user) {
...
exports.forgotPassword = async (req, res) => {
    try {
        ...
        let user = await Society_Registration.findOne({ where: { email , status: 'active' } });
...
exports.changePassword = async (req, res) => {
    try {
        ...
        if (new_password !== confirm_password) {
            ...
            user = await Society_Registration.findOne({ where: { token, status: 'active' } });
            modelName = "Society_Registration";

            // If not found, check in Company_Registration
            if (!user) {
                user = await Company_Registration.findOne({ where: { token, status: 'active' } });
...
exports.verifyOTP = async (req, res) => {
    try {
        ...
        let user = await Society_Registration.findOne({ where: { token, status: 'active' } });
        let modelType = "society";
        ...
        if (!user) {
            return res.status(400).json({ status: 400, message: "Invalid OTP token" });
        }
        ...
        const modelToUpdate = modelType === "society" ? Society_Registration : Company_Registration;
        await modelToUpdate.update({ is_otp_verified: 1 }, { where: { token } });

        // Build response data
        const profileImageKey = modelType === "society" ? "society_profile_img_path" : "company_profile_photo_path";
        ...
        try {
            ...
            const welcomeSubject = modelType === 'society'
            ...
            const welcomeBody = modelType === 'society'
        ...
        // --- WhatsApp onboarding message (separate try/catch) ---
            try {
                ...
                const displayName = modelType === 'society' ? user.society_name : user.company_name;

                const whatsappPayload = {
                    apiKey: process.env.AISENSY_API_KEY,  // store key in .env, not hardcoded
                    campaignName: modelType === 'society'
                ...
                console.error("Failed to send WhatsApp onboarding message:", waErr.message);
            }
...
exports.resendOTP = async (req, res) => {
    try {
        ...
        const modelToCheck = modelType === "society" ? Society_Registration : Company_Registration;
